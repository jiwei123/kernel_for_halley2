...
static int set_delay_sensor_id(struct frizz_input_data *data,uint32_t sensor_id,uint32_t handle_num,unsigned int times)
{
	...
	// sameple 1000 --> interval time(unit : msec), min: 10msec
	ret = 0;
	for(timeout = 0; timeout<50; timeout++){
		udelay(100);
		fifo_size = serial_read_fifo_size(data->client);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	set_delay_sensor_id(data,0x80,0x02,100);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	set_delay_sensor_id(data,0x82,0x2,100);
...
static void mpu6500_input_report_gyro_xyz(struct frizz_input_data *data)
{
	...
	if(enabled_accel){
		...
		input_report_rel(data->gyro_input, REL_X, acc.x);// - data->acc_cal[0]);
		input_report_rel(data->gyro_input, REL_Y, acc.y );//- data->acc_cal[1]);
		input_report_rel(data->gyro_input, REL_Z, acc.z);// - data->acc_cal[2]);
...
static int frizz_input_init(struct frizz_input_data *data)
{
	...
	input_set_capability(data->mcu_input, EV_REL, REL_X);
	input_set_capability(data->mcu_input, EV_REL, REL_Y);
	input_set_capability(data->mcu_input, EV_REL, REL_Z);
	input_set_drvdata(data->mcu_input, data);

	error = input_register_device(data->mcu_input);
	if (error) {
		input_free_device(data->mcu_input);
		return error;
	}

	error = sensors_create_symlink(&data->mcu_input->dev.kobj, idev->name);
	if (error < 0) {
		input_unregister_device(data->mcu_input);
		return error;
	}
	...
	if (error) {
		...
		sensors_remove_symlink(&data->mcu_input->dev.kobj,
			data->mcu_input->name);
		input_unregister_device(data->mcu_input);
		return error;
	}
...
int frizz_ioctl_hardware_reset(struct i2c_client *i2c_client)
{
...
//	mutex_lock(&burn_lock);
	...
	int chip_orientation = g_chip_orientation;
	...
	frizz_ioctl_hardware_reset(i2c_client);
					 ...
					 const struct i2c_device_id *id)
{
	...
	frizz_download_firmware(client,1) ;

...
static void jz_battery_work(struct work_struct *work)
{
	...
	bat->capacity_real = lookup_capacity(is_charging(bat->status), bat->voltage);
...
static void jz_battery_set_resume_time(struct jz_battery *bat)
{
	...
	if (bat->pdata->info.sleep_current == 0){
		...
		interval = ((bat->capacity_show - 10) *
			bat->pdata->info.battery_max_cpt * 36) /
			(bat->pdata->info.sleep_current);
	} else if (bat->capacity_show > 3) {
		interval = ((bat->capacity_show - 3) *
			bat->pdata->info.battery_max_cpt * 36) /
			(bat->pdata->info.sleep_current);
	} else if (bat->capacity_show > 1){
		interval = ((bat->capacity_show - 1) *
			bat->pdata->info.battery_max_cpt * 36) /
			(bat->pdata->info.sleep_current);
	} else if (bat->capacity_show == 1){
	...
	printk(LOG_TAG "set resume time %ld(delta %ld, %ldh %ldm %lds)\n",
			alarm_time.tv_sec, interval,
			interval/3600, (interval/60)%60, interval%60);
 ...
 */
	...
	pr_info(LOG_TAG "time_delta is %ld, %ldh %ldm %lds\n", time_delta,
			time_delta/3600, (time_delta/60)%60, time_delta%60);
	...
	pr_info(LOG_TAG "suspend: %ld last:%ld  next_scan:%d delta:%ld\n",
		   bat->suspend_time, bat->last_update_time, next_scan_time,
		   bat->suspend_time - bat->last_update_time);
	...
	if (bat->status == POWER_SUPPLY_STATUS_FULL ) {
		...
		if ((bat->capacity_real > bat->capacity_show) && (time_delta > 10 * 60)) {
			...
			jz_battery_capacity_rising(bat);
		} else if ((bat->suspend_time - bat->last_update_time) > next_scan_time) {
			jz_battery_capacity_rising(bat);
		} else {
			jz_battery_next_scantime_rising(bat);
...
static int proc_read_status(char *buf, char **start, off_t off,
		int count, int *eof, void *data)
{
	struct jz_battery *bat = (struct jz_battery *)data;

	return sprintf(buf, "%s, cap real: %d, cap show: %d, volt: %dmV, next: %ds\n",
			status2str(bat->status), bat->capacity_real,
			bat->capacity_show, bat->voltage, bat->next_scan_time);
}
		...
		enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if ((POWER_SUPPLY_STATUS_FULL == bat->status) && (100 != bat->capacity_show))
...
static int jz_battery_probe(struct platform_device *pdev)
{
    ...
    struct jz_battery_platform_data *pdata =
            kzalloc(sizeof(struct jz_battery_platform_data),GFP_KERNEL);
    if (!pdata) {
	...
	bat->mem = request_mem_region(bat->mem->start, resource_size(bat->mem), pdev->name);
	if (!bat->mem) {
		ret = -EBUSY;
	...
	res_capacity = proc_create_data("capacity", 0200, root, &capacity_proc_fops, bat);
	if (!res_capacity) {

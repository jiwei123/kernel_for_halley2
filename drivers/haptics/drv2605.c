...
static void vibrator_enable( struct timed_output_dev *dev, int value)
{
    ...
    if (value > 0) {
		...
		value = (value>DRV2605_MAX_TIMEOUT)?DRV2605_MAX_TIMEOUT:value;
   ...
   case HAPTIC_CMDID_PATTERN_RTP:
	{
		...
		if (pDrv2605data->vibration_time > 0) {
			...
			if (pDrv2605data->repeat_times > 0)
				pDrv2605data->repeat_times--;
			if (pDrv2605data->vibration_time > DRV2605_MAX_TIMEOUT)
				pDrv2605data->vibration_time = DRV2605_MAX_TIMEOUT;
			drv2605_change_mode(pDrv2605data, WORK_PATTERN_RTP_ON, DEV_READY);
...
void drv2605_do_suspend(struct drv2605_data *pDrv2605data)
{
	pDrv2605data->should_stop = YES;
	hrtimer_cancel(&pDrv2605data->timer);
	cancel_work_sync(&pDrv2605data->vibrator_work);
...
void drv2605_do_resume(struct drv2605_data *pDrv2605data)
{
	mutex_lock(&pDrv2605data->lock);
	if (pDrv2605data->audio_haptics_enabled == YES) {
		wake_lock(&pDrv2605data->wklock);
...
static int drv2605_suspend(struct device *dev)
{
    ...
    drv2605_do_suspend(pDrv2605data);
...
{
    struct drv2605_data *pDrv2605data = dev_get_drvdata(dev);
    drv2605_do_resume(pDrv2605data);
...
#if 0
	/* Next two lines make motor vibrate */
	drv2605_set_go_bit(pDrv2605data, GO);
...
{
	int ret = 0;
	ret = i2c_add_driver(&drv2605_driver);
	return ret;
}

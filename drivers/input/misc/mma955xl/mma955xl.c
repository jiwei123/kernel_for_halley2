...
static int mma955xl_send_command(struct mma955xl_data *pdata,int appid , int cmd , int offset , char *data, int size)
{
	...
	pcmd->offset = offset;
	pcmd->count = size;
	if(pcmd->cmd.bits.cmd != MMA955XL_FCI_CONFIG_R && pcmd->cmd.bits.cmd != MMA955XL_FCI_DATA_R)   // Config write Cmd.
	{
		if(size < 20)
		{
	...
	do
	{
		...
		if (ret <= 0)
		{
		   ...
		   if(pcmd->cmd.bits.cmd == MMA955XL_FCI_CONFIG_R || pcmd->cmd.bits.cmd == MMA955XL_FCI_DATA_R )
			{
				ret = pdata->read(pdata, (u8 *)presp, size + sizeof(struct mma955xl_rep_h_t));
			}
			else
			{
				ret = pdata->read(pdata, (u8 *)presp, 4);
			}
			if (presp->cc == 0x01 && presp->error == 0x00 && presp->bytes_read == presp->bytes_rqst)
			{
				if(pcmd->cmd.bits.cmd == MMA955XL_FCI_CONFIG_R || pcmd->cmd.bits.cmd == MMA955XL_FCI_DATA_R)
				{
...
static int mma955xl_write(struct mma955xl_data *pdata,u8 *mb_frame , int buf_len)
{
...
static int mma955xl_set_nr(struct mma955xl_data *pdata)
{
	...
	u8 afe_frame[32]  = {0x06,0x20,0x00,0x0A,0x02,0x10,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
...
{
	int ret;
	u8 pedo_conf[32] = {0x15,0x20,0x00, 0x10, 0x0C, 0xE0, 0x13, 0x20,0x00,0x5A,0xE0,0x64,0xB4, 0x4E,0x01,0x83,0x05,0x01,0x00,0x00};
    ret = mma955xl_write(pdata,pedo_conf,sizeof(pedo_conf));
	if(ret < 0)
		return ret;
	return 0;
}
static int mma955xl_pedometer_status(struct mma955xl_data *pdata,struct mma955xl_pedometer_status *pstatus){
	int ret;
...
static int mma955xl_read_data(struct mma955xl_data *pdata, struct mma955xl_data_axis *data) {
	int ret;
	u8 frame_nr_read[256] = {0x06,0x30,0x00,0x06};
	u8 * repn_data;
	ret = mma955xl_read(pdata , frame_nr_read, sizeof(frame_nr_read));
	if(ret < 0)
	{
		printk(KERN_ERR "mma955xl %s read afe 3-axies data error \n",__FUNCTION__);
		return -EIO;
	}
	repn_data = MMA955XL_DATA_OF_RESPONSE(frame_nr_read);
	data->x = ((repn_data[0] << 8) & 0xff00) | repn_data[1];
	data->y = ((repn_data[2] << 8) & 0xff00) | repn_data[3];
	data->z = ((repn_data[4] << 8) & 0xff00) | repn_data[5];
	return 0;
}
static void mma955xl_report_data(struct mma955xl_data* pdata)
{
	int ret = -1; u8 active_level;
	struct input_polled_dev * poll_dev = pdata->poll_dev;
	struct mma955xl_data_axis data;
	struct mma955xl_pedometer_status status;
	if(pdata->wsmode == MMA955XL_MODE_WAKE){
		ret = mma955xl_read_data(pdata,&data);
		if (!ret)
		{
			input_report_abs(poll_dev->input, ABS_X, data.x << 2);
			input_report_abs(poll_dev->input, ABS_Y, data.y << 2);
			input_report_abs(poll_dev->input, ABS_Z, data.z << 2);
		...
		if(!ret)
		{
			...
			active_level = (status.status >> 8 & 0x07);
			if(active_level < 2 || active_level > 4 )
				active_level = 0;
			input_report_abs(poll_dev->input, ABS_RY,active_level);  //significant motion
			input_sync(poll_dev->input);
		}
...
static DEVICE_ATTR(enable_pedometer, S_IWUSR | S_IRUGO,
		   mma955xl_enable_show, mma955xl_enable_store);
static DEVICE_ATTR(position, S_IWUSR | S_IRUGO,
		   mma955xl_position_show, mma955xl_position_store);
static DEVICE_ATTR(poll_interval_pedometer, S_IWUSR | S_IRUGO,
		   mma955xl_poll_interval_show, mma955xl_poll_interval_store);

static struct attribute *mma955xl_attributes[] = {
	&dev_attr_enable_pedometer.attr,
	&dev_attr_position.attr,
	&dev_attr_poll_interval_pedometer.attr,
	NULL
};

static const struct attribute_group mma955xl_attr_group = {
	.attrs = mma955xl_attributes,
};
...
int  mma955xl_device_init(struct mma955xl_data *pdata)
{
	...
	result = sysfs_create_group(&idev->dev.kobj, &mma955xl_attr_group);
	if (result) {
...
error_init:
	sysfs_remove_group(&idev->dev.kobj, &mma955xl_attr_group);
err_create_sysfs:
...
err_alloc_poll_device:
	kfree(pdata);
err_gpio_2:
    gpio_free(p->sensor_int0_n);
err_gpio_1:
	gpio_free(p->ap_wakeup_mcu);
err_gpio_0:
	gpio_free(p->mcu_wakeup_ap);
err_out:
	return result;
}

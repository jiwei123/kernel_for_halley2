...
#ifdef CONFIG_FOCALTECH_GESTRUE
	ret = i2c_smbus_read_i2c_block_data(ts->client, 0xd0, 1, &state);
	if( state ==1 )
		fts_read_Gestruedata(ts);
#endif
	fts_update_data(ts);
...
static void focaltech_ts_do_suspend(struct fts_data *fts)
{
	...
	fts->is_suspend = 1;
	fts_reset(fts);
	fts_write_reg(fts->client, FTS_REG_PMODE, 0x00);
	fts_read_reg(fts->client, FTS_REG_PMODE, &ret);
	printk(KERN_DEBUG "==%s== read FTS_REG_PMODE[0x%0X] = 0x%X gestrue_state = %d\n", __FUNCTION__, FTS_REG_PMODE, ret, fts->gestrue_state);

	if(fts->gestrue_state) {
		ret = fts_write_reg(fts->client, 0xd0, 0x01);
		if(ret < 0){
			PRINT_ERR("==fts_suspend FTS_GESTRUE==  fts_write_reg fail ret = %d\n", ret);
		}

		if (fts_updateinfo_curr.CHIP_ID==0x54) {
			fts_write_reg(fts->client, 0xd1, 0xff);
			fts_write_reg(fts->client, 0xd2, 0xff);
			fts_write_reg(fts->client, 0xd5, 0xff);
			fts_write_reg(fts->client, 0xd6, 0xff);
			fts_write_reg(fts->client, 0xd7, 0xff);
			fts_write_reg(fts->client, 0xd8, 0xff);
		}
	} else {
		ret = fts_write_reg(fts->client, FTS_REG_PMODE, PMODE_HIBERNATE);
		if(ret < 0){
			PRINT_ERR("==fts_suspend==  fts_write_reg fail ret= %d\n", ret);
		}
...
static void focaltech_ts_do_resume(struct fts_data *fts)
{
		printk(KERN_DEBUG "==%s==\n", __FUNCTION__);
		...
		fts->is_suspend = 0;
		if(fts->gestrue_state) {
			fts_write_reg(fts->client,0xD0,0x00);

		} else {
			regulator_enable(fts->vcc_reg);
			msleep(100);
			enable_irq(fts->client->irq);
		}
...
static int tp_notifier_callback(struct notifier_block *self,unsigned long event, void *data)
{
	...
	fts = container_of(self, struct fts_data, tp_notif);

	if(event == FB_EVENT_BLANK){
		if(mode)
			focaltech_ts_do_suspend(fts);
		else
			focaltech_ts_do_resume(fts);
	}
	return 0;
}

static void focaltech_register_notifier(struct fts_data *fts)
{
	memset(&fts->tp_notif,0,sizeof(fts->tp_notif));
	fts->tp_notif.notifier_call = tp_notifier_callback;
	/* register on the fb notifier  and work with fb*/
	fb_register_client(&fts->tp_notif);
}

static void focaltech_unregister_notifier(struct fts_data *fts)
{
	fb_unregister_client(&fts->tp_notif);
}
...
static int fts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	focaltech_register_notifier(fts);
...
static int fts_remove(struct i2c_client *client)
{
	...
	focaltech_unregister_notifier(fts);

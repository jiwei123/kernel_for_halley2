...
#define GPIO_LCD_BLK_EN     GPIO_PA(3)
#define GPIO_MIPI_RST_N     GPIO_PC(9)
#define DSI_TE_GPIO         GPIO_PC(19)
#define VCC_LCD_1V8_NAME    BUCK3_NAME
#define VCC_LCD_2V8_NAME    BUCK4_NAME

static struct regulator *lcd_vcc_reg = NULL;
static struct regulator *lcd_io_reg = NULL;
static bool is_init = 0;

int jdi_lpm013m081a_power_init(struct lcd_device *lcd)
{
	int ret = 0;
...
int jdi_lpm013m081a_reset(struct lcd_device *lcd)
{
...
int jdi_lpm013m081a_power_on(struct lcd_device *lcd, int enable)
{
	int ret = 0;

	if(!is_init && jdi_lpm013m081a_power_init(lcd))
		return -EFAULT;

	printk (KERN_ERR "lcd vcc is enabled : %d\n",regulator_is_enabled(lcd_vcc_reg));
	printk (KERN_ERR "lcd vio is enabled : %d\n",regulator_is_enabled(lcd_io_reg));
...
struct lcd_platform_data jdi_lpm013m081a_data = {
	.reset    = jdi_lpm013m081a_reset,
	.power_on = jdi_lpm013m081a_power_on,
	.lcd_enabled = 1, // lcd panel was enabled from uboot
};

struct mipi_dsim_lcd_device jdi_lpm013m081a_device={
	.name = "jdi_lpm013m081a-lcd",
	.id   = 0,
	.platform_data = &jdi_lpm013m081a_data,
};

unsigned long jdi_lpm013m081a_cmd_buf[]= {
	0x2C2C2C2C,
};
...
struct jzfb_platform_data jzfb_pdata = {
	...
	.smart_config.write_gram_cmd = jdi_lpm013m081a_cmd_buf,
	.smart_config.length_cmd = ARRAY_SIZE(jdi_lpm013m081a_cmd_buf),

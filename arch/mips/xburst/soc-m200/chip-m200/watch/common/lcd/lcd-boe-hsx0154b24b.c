...
struct boe_hsx0154b24b_power{
	struct regulator *vlcdio;
	struct regulator *vlcdvcc;
	int inited;
};

static struct boe_hsx0154b24b_power lcd_power = {
	NULL,
    NULL,
    0
};

int boe_hsx0154b24b_power_init(struct lcd_device *ld)
{
    int ret ;
    if(GPIO_LCD_RST > 0){
...
int boe_hsx0154b24b_power_reset(struct lcd_device *ld)
{
	if (!lcd_power.inited)
		return -EFAULT;
...
int boe_hsx0154b24b_power_on(struct lcd_device *ld, int enable)
{
	if (!lcd_power.inited && boe_hsx0154b24b_power_init(ld))
		return -EFAULT;
	...
	if (enable) {
		...
		boe_hsx0154b24b_power_reset(ld);
...
struct lcd_platform_data boe_hsx0154b24b_pdata = {
	.reset    = boe_hsx0154b24b_power_reset,
	.power_on = boe_hsx0154b24b_power_on,
};

/* LCD Panel Device */
struct platform_device boe_hsx0154b24b_device = {
	.name		= "boe_hsx0154b24b_slcd",
	.dev		= {
		.platform_data	= &boe_hsx0154b24b_pdata,
	},
};
#if 0//9340L
static struct smart_lcd_data_table boe_hsx0154b24b_data_table[] = {
    /* LCD init code */
    ...
    {SMART_CONFIG_DATA, 0xb8},

    {SMART_CONFIG_CMD, 0xce},
    {SMART_CONFIG_DATA, 0xa0},

    {SMART_CONFIG_CMD, 0x36},
    {SMART_CONFIG_DATA, 0x48}, //88

    {SMART_CONFIG_CMD, 0x3a},
    {SMART_CONFIG_DATA, 0x55}, //5-6-5 0x05
...
static struct smart_lcd_data_table boe_hsx0154b24b_data_table[] = {
    ...
    {SMART_CONFIG_CMD, 0x3a},
#if 0//defined(CONFIG_SLCD_TRULY_18BIT)  //if 18bit/pixel unusual. try to use 16bit/pixel
...
struct jzfb_platform_data jzfb_pdata = {
	...
	.smart_config.length_data_table =  ARRAY_SIZE(boe_hsx0154b24b_data_table),
	.smart_config.data_table = boe_hsx0154b24b_data_table,
